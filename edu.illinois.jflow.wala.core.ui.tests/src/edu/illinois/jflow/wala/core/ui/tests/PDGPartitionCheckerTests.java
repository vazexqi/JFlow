...
public class PDGPartitionCheckerTests extends JFlowTest {
	...
	@Test
	public void testRecipe_checkHeapInterference() throws IOException, InvalidClassFileException, CancelException {
		IR ir= retrieveMethodIR(constructFullyQualifiedClass(), "main", "[Ljava/lang/String;", "V");
		ProgramDependenceGraph pdg= ProgramDependenceGraph.make(ir, engine.buildClassHierarchy());
		List<List<Integer>> selections= selectionFromArray(new int[][] { { 7 }, { 9 }, { 12, 13 }, { 18 } });
		PDGPartitionerChecker checker= PDGPartitionerChecker.makePartitionChecker(pdg, selections);
		checker.computeHeapDependency(callGraph, engine.getPointerAnalysis());
		checker.checkInterference();
		assertFalse(checker.hasInterference());
	...
	@Test
	public void testRecipePartialTransfer_checkHeapInterference() throws IOException, InvalidClassFileException, CancelException {
		IR ir= retrieveMethodIR(constructFullyQualifiedClass(), "main", "[Ljava/lang/String;", "V");
		ProgramDependenceGraph pdg= ProgramDependenceGraph.make(ir, engine.buildClassHierarchy());
		List<List<Integer>> selections= selectionFromArray(new int[][] { { 7 }, { 9, 10 }, { 14 }, { 18 } });
		PDGPartitionerChecker checker= PDGPartitionerChecker.makePartitionChecker(pdg, selections);
		checker.computeHeapDependency(callGraph, engine.getPointerAnalysis());
		checker.checkInterference();
		assertFalse(checker.hasInterference());
	...
	@Test
	public void testProject10_checkHeapAnalysis() throws IOException, InvalidClassFileException, CancelException {
		IR ir= retrieveMethodIR(constructFullyQualifiedClass(), "method", "", "V");
		ProgramDependenceGraph pdg= ProgramDependenceGraph.make(ir, engine.buildClassHierarchy());
		List<List<Integer>> selections= selectionFromArray(new int[][] { { 20 }, { 22, 23 }, { 27, 28 }, { 32 } });
		PDGPartitionerChecker checker= PDGPartitionerChecker.makePartitionChecker(pdg, selections);
	...
	@Test
	public void testProject12_checkHeapAnalysis() throws IOException, InvalidClassFileException, CancelException {
		IR ir= retrieveMethodIR(constructFullyQualifiedClass(), "main", "[Ljava/lang/String;", "V");
		ProgramDependenceGraph pdg= ProgramDependenceGraph.make(ir, engine.buildClassHierarchy());
		List<List<Integer>> selections= selectionFromArray(new int[][] { { 5 }, { 7, 8 }, { 12 } });
		PDGPartitionerChecker checker= PDGPartitionerChecker.makePartitionChecker(pdg, selections);
		checker.computeHeapDependency(callGraph, engine.getPointerAnalysis());
		checker.checkInterference();
		assertFalse(checker.hasInterference());
